...
- **Transactional Integrity:**
  - Use Firestore transactions for multi-document mutations such as Morning Check-In (rollover + day session creation) and End-of-Day Check-Out.
...
2. **With `@devops-engineer`:** Rely on local emulator environment configurations (`FIRESTORE_EMULATOR_HOST=localhost:8080`) and ensure changes pass Docker multi-stage builds.
3. **Pre-Flight Self-Verification (Before `@tester` Hand-off):**
   - [ ] Run Go unit tests: `go test -v ./...`
   - [ ] Run Frontend tests & type-checks: `cd frontend && npm test`
   - [ ] Verify build compiles cleanly: `cd frontend && npm run build && go build ./cmd/server`
